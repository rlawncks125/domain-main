...
const items = [색상(), 문자(), 숫자(), list(), map(), 관리()];

function 색상(): CodeReutrnType {
  return {
    title: "색상(RGB / HSL / Opacity) 함수",
    code: [
      {
        lang: CodeLanguages.scss,
        code: `mix($color1, $color2) : 두 개의 색을 섞습니다.
...
function 문자(): CodeReutrnType {
  return {
    title: "문자(String) 함수",
    code: [
      {
        lang: CodeLanguages.scss,
        code: `unquote($string) : 문자에서 따옴표를 제거합니다.
...
function 숫자(): CodeReutrnType {
  return {
    title: "숫자(Number) 함수",
    code: [
      {
        lang: CodeLanguages.scss,
        code: `percentage($number) : 숫자(단위 무시)를 백분율로 변환합니다.
...
function list(): CodeReutrnType {
  return {
    title: "List 함수",
    code: [
      {
        lang: CodeLanguages.scss,
        code: `모든 List 내장 함수는 기존 List 데이터를 갱신하지 않고 새 List 데이터를 반환합니다.
...
function map(): CodeReutrnType {
  return {
    title: "Map 함수",
    code: [
      {
        lang: CodeLanguages.scss,
        code: `모든 Map 내장 함수는 기존 Map 데이터를 갱신하지 않고 새 Map 데이터를 반환합니다.
...
function 관리(): CodeReutrnType {
  return {
    title: "관리(Introspection) 함수",
    code: [
      {
        lang: CodeLanguages.scss,
        code: `variable-exists(name) : 변수가 현재 범위에 존재하는지 여부를 반환합니다.(인수는 $없이 변수의 이름만 사용합니다.)
