...
function 후손(): CodeReutrnType {
  return {
    title: "후손 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `(선택자1) (선택자2) : 선택자1의 모든 하위 요소(자식, 자식의 자식, ...) 중 선택자2를 선택한다.`,
      },
    ],
  };
}
function 다중(): CodeReutrnType {
  return {
    title: "다중 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `(선택자1)(선택자2) : 선택자1이면서 선태작2인 요소를 선택한다.`,
      },
    ],
  };
}
function 자손(): CodeReutrnType {
  return {
    title: "자손 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `(선택자1) > (선택자2) : 선택자1 바로 밑의 자식 중 선택자2를 선택한다.`,
      },
    ],
  };
}
function 속성(): CodeReutrnType {
  return {
    title: "속성 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `[속성] : 태그에 해당 속성이 명시되어 있는 요소를 선택한다.

[속성=문자열] : 태그에 해당 속성이 명시되어 있고 그 값이 해당 문자열인 요소를 선택한다.

[속성~=문자열] : 해당 속성의 값에 해당 문자열이 완전한 단어로(즉, 앞뒤가 띄어쓰기로 구분되어서) 포함되어 있는 요소를 선택한다.

[속성|=문자열] : 해당 속성의 값이 해당 문자열이거나 그 뒤에 하이픈(-)이 붙는 것으로 시작하는 요소를 선택한다.

[속성*=문자열] : 해당 속성의 값에 해당 문자열이 포함되어 있는 요소를 선택한다.
...
function 동위(): CodeReutrnType {
  return {
    title: "동위 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `(선택자1) + (선택자2) : 선택자1의 형제 중 바로 다음에 오는 선택자2 하나를 선택한다.
...
function 구조(): CodeReutrnType {
  return {
    title: "구조 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `:first-child = 형제 중 첫 번째로 오는 요소를 선택한다.
...
function 반응(): CodeReutrnType {
  return {
    title: "반응 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `:active = 클릭하고 있는 요소를 선택한다.
:hover = 커서가 올라와 있는 요소를 선택한다.`,
      },
    ],
  };
}
function 상태(): CodeReutrnType {
  return {
    title: "상태 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `:checked = 체크되어 있는 <input> 체크박스, 라디오 버튼이나 드롭다운 메뉴의 선택되어 있는 선택지(<option>)를 선택한다.
...
function 링크(): CodeReutrnType {
  return {
    title: "링크 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `:link = href 속성이 명시된[4] <a> 요소를 선택한다.
:visited = 방문했던 링크가 걸린 <a> 요소를 선택한다.`,
      },
    ],
  };
}
function 부정(): CodeReutrnType {
  return {
    title: "부정 선택자",
    code: [
      {
        lang: CodeLanguages.css,
        code: `:not(선택자) = 해당 선택자에 해당하지 않는 요소들을 선택한다`,
      },
    ],
  };
}
function 가상요소(): CodeReutrnType {
  return {
    title: "가상요소",
    code: [
      {
        lang: CodeLanguages.css,
        code: `::first-letter = 첫 번째 글자를 선택한다. ::before의 영향을 받는다. 특정한 CSS 속성만 사용할 수 있는 제한이 있다(영어).[6]
